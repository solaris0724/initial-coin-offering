    ...
    describe('finalizing the crowdsale', () => {
        ...
        describe('when the goal is reached', async() => {
            ...
            it('handles goal reached', async() => {
                ...
                // Founders
                let foundersBalance = await token.balanceOf(foundersFund);
                foundersBalance = foundersBalance / (10 ** decimals);

                assert.equal(foundersBalance.toString(), foundersAmount.toString());

                // Foundation
                let foundationBalance = await token.balanceOf(foundationFund);
                foundationBalance = foundationBalance / (10 ** decimals);

                assert.equal(foundationBalance.toString(), foundationAmount.toString());

                // Partners
                let partnersBalance = await token.balanceOf(partnersFund);
                partnersBalance = partnersBalance / (10 ** decimals);
